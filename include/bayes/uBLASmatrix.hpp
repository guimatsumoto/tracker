...
namespace Bayesian_filter_matrix {
    ...
    enum EmptyTag {
        Empty
    }; // Tag type used for empty matrix constructor
...
#define BOOST_UBLAS_TYPENAME typename
    ...
    namespace detail // Lots of implementation detail
    {
        ...
        public:
            ...
            template <class E>
            FMVec& operator=(const ublas::vector_expression<E>& r) { // Expression assignment; may be dependent on r
                VecBase::operator=(r);
                return *this;
            }
        ...
        public:
            ...
            template <class E>
            FMMatrix& operator=(const ublas::matrix_expression<E>& r) { // Expression assignment; may be dependent on r
    ...
    diag(FMMatrix<Base>& M, std::size_t n) { // Return a vector proxy to the first n diagonal elements of M
    ...
    diag(const FMMatrix<Base>& M, std::size_t n) { // Return a vector proxy to the first n diagonal elements of M
    ...
    template <class Base>
    void identity(FMMatrix<Base>& I) { // Set I to generalised Identity matrix. Clear I and set diag(I) to one
        I.clear();
    ...
    struct prod_expression_result { // Provide ET result E1E2T_type of prod(matrix_expression<E1>,trans(matrix_expression<E2>)
